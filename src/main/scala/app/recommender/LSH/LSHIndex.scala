...
class LSHIndex(data: RDD[(Int, String, List[String])], seed: IndexedSeq[Int]) extends Serializable {
  ...
  private val hashPartitioner = new HashPartitioner(100)
  ...
  : RDD[(IndexedSeq[Int], T, List[(Int, String, List[String])])] =
    queries
      ...
      .join(getBuckets())
      .map {
        case (_, ((payload, signature), candidates)) =>
          (signature, payload, candidates.filter(x => signature == minhash.hash(x._3)))
      }
}
